...
#./prepare_data_perm_batch.sh 4 12 "/nobackup/users/yangzhan/ssl-disentangle/feat_1" "/nobackup/users/yangzhan/ssl-disentangle/label_1" "/nobackup/users/yangzhan/ssl-disentangle/km_v04/km_100_12_1" "/nobackup/users/kzqian/ssl-disentangle/v05/checkpoints/checkpoint.best_loss_2.2779.pt"
...
CUDA_VISIBLE_DEVICES=$(bc <<< "${rank}/${ntasks}") python dump_hubert_feature_perm_batch.py $meta_dir $spk2info "train" $ckpt_path $layer ${nshard} ${rank} 64 18 $feat_dir --disable_tqdm True &
pids+=($!)
done
