...
#./prepare_data_perm_batch.sh 4 12 "/nobackup/users/yangzhan/ssl-disentangle/feat_1" "/nobackup/users/yangzhan/ssl-disentangle/label_1" "/nobackup/users/yangzhan/ssl-disentangle/km_v05/km_100_12_1" False
...
CUDA_VISIBLE_DEVICES=$(bc <<< "${rank}/${ntasks}") python dump_hubert_feature_perm_batch.py $meta_dir $spk2info "train" $ckpt_path $layer ${nshard} ${rank} 64 18 $feat_dir --disable_tqdm $flag &
pids+=($!)
done
...
pids=()
for rank in $(seq 0 $((nshard - 1))); do
CUDA_VISIBLE_DEVICES=$(bc <<< "${rank}/${ntasks}") python dump_km_label_batch.py $feat_dir "train" "${km_dir}" ${nshard} ${rank} $lab_dir --disable_tqdm $flag &
pids+=($!)
done
i=0; for pid in "${pids[@]}"; do wait ${pid} || ((++i)); done
end=`date +%s`
