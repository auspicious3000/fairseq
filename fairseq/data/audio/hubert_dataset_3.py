...
def load_audio(manifest_path_1, manifest_path_2, max_keep, min_keep):
    n_long, n_short = 0, 0
    names_1, names_2, inds, sizes = [], [], [], []
    with open(manifest_path_1) as f_1, open(manifest_path_2) as f_2:
        root_1 = f_1.readline().strip()
        root_2 = f_2.readline().strip()
        for ind, (line_1, line_2) in enumerate(zip(f_1, f_2)):
            items_1 = line_1.strip().split("\t")
            items_2 = line_2.strip().split("\t")
            ...
            if min_keep is not None and sz < min_keep:
                ...
                names_1.append(items_1[0])
                names_2.append(items_2[0])
                inds.append(ind)
                sizes.append(sz)
    tot = ind + 1
    logger.info(
        (
            ...
            f"loaded {len(names_1)}, skipped {n_short} short and {n_long} long, "
            f"longest-loaded={max(sizes)}, shortest-loaded={min(sizes)}"
        )
    )
    return root_1, names_1, root_2, names_2, inds, tot, sizes
...
class HubertDataset_3(FairseqDataset):
    def __init__(
        self,
        manifest_path_1: str,
        manifest_path_2: str,
        ...
        self.split = manifest_path_1.split('/')[-1][:-4]
        ...
        self.audio_root_1, self.audio_names_1, self.audio_root_2, self.audio_names_2, inds, tot, self.sizes = load_audio(
            manifest_path_1, manifest_path_2, max_keep_sample_size, min_keep_sample_size
        )
    ...
    def get_audio_0(self, index):   
        # perm + perm
        fileName = self.audio_names_1[index]
        fileLen = self.sizes[index]
        spk = fileName.split('/')[1]
        wav_path = os.path.join(self.audio_root_1, fileName)
    ...
    def get_audio(self, index):
        ...
        if case == 0:
            ...
            return self.get_audio_1(index)
        elif case == 2:
            return self.get_audio_2(index)
        elif case == 3:
            return self.get_audio_3(index)
        else:
