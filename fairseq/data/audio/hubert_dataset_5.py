...
class HubertDataset_5(FairseqDataset):
    def __init__(
        self,
        manifest_path: str,
        sample_rate: float,
    ...
    def get_label(self, index, label_idx):
        ...
        label = self.cdb[label.long()-num_special_token]
        return label
    ...
    def collater_frm_label(
        ...
        # pad the list of T x D tensors
        lengths = torch.LongTensor([len(t) for t in targets])
        ntokens = lengths.sum().item()
        B = len(targets)
        _, H = targets[0].shape
        max_length = torch.max(lengths)
        targets_ = torch.zeros(B, max_length, H).fill_(pad).to(targets[0])
        for i, t in enumerate(targets):
            targets_[i, :len(t)] = t
            
        return targets_, lengths, ntokens
