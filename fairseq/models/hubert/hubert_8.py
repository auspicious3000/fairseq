...
from fairseq.tasks.hubert_8_pretraining import (
    HubertPretrainingConfig_8,
    HubertPretrainingTask_8,
)
...
@dataclass
class HubertConfig_8(FairseqDataclass):
...
@register_model("hubert_8", dataclass=HubertConfig_8)
class HubertModel_8(BaseFairseqModel):
    def __init__(
        self,
        cfg: HubertConfig_8,
        task_cfg: HubertPretrainingConfig_8,
    ...
    @classmethod
    def build_model(cls, cfg: HubertConfig_8, task: HubertPretrainingTask_8):
        """Build a new model instance."""

        model = HubertModel_8(cfg, task.cfg, task.dictionaries)
    ...
    def forward(
        ...
        if not self.skip_masked:
            ...
            target_m_list = [t[mask_indices] for t in target_list]
        else:
            logit_m_list = [None for _ in target_list]
            target_m_list = [None for _ in target_list]
        ...
        if not self.skip_nomask:
            ...
            target_u_list = [t[nomask_indices] for t in target_list]
        else:
            logit_u_list = [None for _ in target_list]
            target_u_list = [None for _ in target_list]
    ...
    def get_targets(self, net_output, is_masked=True):
        if is_masked:
            targets_list = net_output["target_m_list"]
        else:
            targets_list = net_output["target_u_list"]
        targets_list = [x.float() for x in targets_list if x is not None]
        return targets_list
